...
try:
    ...
    try:
        # Import tương đối
        from ..services.vimrc_service import vimrc_service
        from ..services.openai_service import openai_service
        from ..services.gemini_service import gemini_service
        from ..services.document_store import document_store
        from ..services.product_service import product_service
        from ..core.config import settings
        from ..core.ai_config import AIProvider, ai_settings, get_all_model_names
        from ..schemas.ai_schemas import SmartQARequest, SmartQAResponse
...
async def smart_qa(request: SmartQARequest):
    ...
    try:
        ...
        is_product_query = await product_service.is_product_query(question)
        
        if is_product_query:
            logger.info(f"SmartQA: Phát hiện câu hỏi liên quan đến sản phẩm: {question}")
            # Xử lý câu hỏi về sản phẩm
            product_response = await product_service.process_product_query(question)
...
async def send_message(request: ChatRequest):
    ...
    try:
        ...
        if is_product_query:
            logger.info(f"Phát hiện câu hỏi liên quan đến sản phẩm: {question}")
            # Xử lý câu hỏi về sản phẩm
            product_response = await product_service.process_product_query(question)
            
            # Lấy provider mặc định từ cấu hình
            default_provider = AIProvider.GEMINI
            if provider:
                response_provider = provider
            else:
                response_provider = default_provider
